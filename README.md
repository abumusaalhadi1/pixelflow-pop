...
This file acts as the browser interface for the WebAssembly application.
...
* the `{{{ SCRIPT }}}` placeholder where the loader script is injected

The HTML file embeds and launches the WebAssembly program.
...
JavaScript Loader (.js)
        ↓
HTML Page (shell.html)
        ↓
Browser Canvas renders the simulation
```
